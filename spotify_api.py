...
def get_playlist_info(link, trackname_convention, mode, token):
    ...
    response = requests.get(
        f"https://api.spotidownloader.com/metadata/{mode}/{playlist_id}?token={token}",
        headers=CUSTOM_HEADER,
    )
    ...
    track_list = []
    response = requests.get(
        f"https://api.spotidownloader.com/tracks/{mode}/{playlist_id}?token={token}",
        headers=CUSTOM_HEADER,
    )
    ...
    while next_offset:
        response = requests.get(
            f"https://api.spotidownloader.com/tracks/{mode}/{playlist_id}?offset={next_offset}&token={token}",
            headers=CUSTOM_HEADER,
        )
        tracks_data = response.json()
        track_list.extend(tracks_data["trackList"])
